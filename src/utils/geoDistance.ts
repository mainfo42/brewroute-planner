...
const CITY_COORDINATES: Record<string, LatLng> = {
  ...
  'decorah, ia': { lat: 43.3033, lng: -91.7857 },
 ...
 * - Normalized partial lookup (e.g. "Quebec, Canada" -> "quebec, canada")
 */
export function resolveCoordinates(locationStr: string, fallbackCoord?: LatLng): LatLng {
  if (!locationStr || !locationStr.trim()) {
    return fallbackCoord || { lat: 44.4759, lng: -73.2121 }; // Default Burlington, VT
  }
  ...
  // Try substring matching for known major cities
  ...
  // Check if city name is in first part of comma: "Quebec, QC" -> "quebec"
  const firstPart = normalized.split(',')[0].trim();
  if (CITY_COORDINATES[firstPart]) {
    return CITY_COORDINATES[firstPart];
  }

  // Return provided fallback or default
...
export function calculateDrivingTransit(from: LatLng, to: LatLng): TransitEstimate {
  ...
  // Highways/rural interstates curve ~20-28% longer than straight geodesic lines
